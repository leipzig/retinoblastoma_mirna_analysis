...
solexaDataDir<-"/nas/is1/leipzig/Ganguly/doSolexa/"
ionDataDir<-"/nas/is1/leipzig/Ganguly/RB525T_PGM/"
bamDirectory<-"bam/"
outputDir="/nas/is1/leipzig/Ganguly/gangulyRBhi/results/"
...
counts$WERI<-counts[,'31s8_WERI']+counts[,'36s2_WERI']
counts$Y79<-counts[,'36s1_Y79']+counts[,'36s1_Y79']
counts<-counts[,!str_detect(colnames(counts),'^3')]
#this messes up the id.var so matrix doesn't work

#42s1_nrml  RB494N RB494T RB495N  RB495T RB498N RB498T RB517T  RB525T   WERI    Y79
conds<-c('N','N','T','N','T','N','T','T','T','T','T','T')
individual<-c(1,2,2,3,3,4,4,5,6,7,8,6)
replicate<-c(1,1,2,1,2,1,2,2,2,2,2,2)
type<-c('IIfx','HS','HS','HS','HS','HS','HS','HS','HS','IIfx','IIfx','PGM')
pdata<-data.frame(condition=conds,replicate=replicate,type=type,individual=individual)
rownames(pdata)<-c('42s1_nrml','RB494N','RB494T','RB495N','RB495T','RB498N','RB498T','RB517T','RB525T','WERI','Y79','RB525T_PGM')
countsMatrix<-as.matrix(counts[,-1])
rownames(countsMatrix)<-counts$miRNA
colnames(countsMatrix)<-colnames(counts[,-1])



countsAboveThreshold<-subset(countsMatrix,rowSums(countsMatrix)>minCount)



cds <- newCountDataSet( countsAboveThreshold, pdata$condition )
...
#sharingMode="fit-only" gene-est-only
...
\begin{itemize}
  \item Illumina HiSeq
  \begin{enumerate}
     \item RB494N \Sexpr{formatC(bamcounts$RB494N$records, format="d", big.mark=',')} alignments
     \item RB494T \Sexpr{formatC(bamcounts$RB494N$records, format="d", big.mark=',')} alignments
     \item RB495N \Sexpr{formatC(bamcounts$RB495N$records, format="d", big.mark=',')} alignments
     \item RB495T \Sexpr{formatC(bamcounts$RB495T$records, format="d", big.mark=',')} alignments
     \item RB498N \Sexpr{formatC(bamcounts$RB498N$records, format="d", big.mark=',')} alignments
     \item RB498T \Sexpr{formatC(bamcounts$RB498T$records, format="d", big.mark=',')} alignments
     \item RB517T \Sexpr{formatC(bamcounts$RB517T$records, format="d", big.mark=',')} alignments
     \item RB525T \Sexpr{formatC(bamcounts$RB525T$records, format="d", big.mark=',')} alignments
  \end{enumerate}
  \item Illumina IIfx
    \begin{enumerate}
       \item WERI 31s8 and 36s2 technical replicates \Sexpr{formatC(bamcounts[['31s8_WERI']]$records + bamcounts[['36s2_WERI']]$records, format="d", big.mark=',')} alignments
       \item Y79 36s1 and 36s2 technical replicates \Sexpr{formatC(bamcounts[['36s1_Y79']]$records+bamcounts[['36s2_Y79']]$records, format="d", big.mark=',')} alignments
       \item 42s1\_nrml \Sexpr{formatC(bamcounts[['42s1_nrml']]$records, format="d", big.mark=',')} alignments
    \end{enumerate}
    \item Ion Torrent PGM
    \begin{enumerate}
    \item RB525T\_PGM \Sexpr{formatC(bamcounts$RB525T_PGM$records, format="d", big.mark=',')} alignments
    \end{enumerate}
    \end{itemize}  
...
-f FASTQ_FILE.fq -m -l 17 -h 60 -t 65 -o sam -o FullNW
\end{verbatim}
...
heatmap(countsAboveThreshold[substr(rownames(countsAboveThreshold),0,7)=='hsa-let',],Rowv=NA,keep.dendro=FALSE,labRow=NULL)
@
